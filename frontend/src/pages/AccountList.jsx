...
import { accountApi, accountGroupApi } from '../services/api';
...
const STATUS_TABS = [
  ...
  { value: 'LOGIN_THANH_CONG', label: '📤 Upload Thành Công', color: '#10b981' },
...
// ── Bulk Bar ─────────────────────────────────────────────────────────────────
function BulkBar({ selected, onClear, onRefresh, onCheckLive, clChecking }) {
  ...
  const handleMarkUsed = async () => {
    ...
    setBusy(true);
    try {
      const res = await accountApi.bulkAction(ids, 'mark_used');
  ...
  const handleSetStatus = async () => {
    ...
    setBusy(true);
    try {
      const res = await accountApi.bulkAction(ids, 'set_status', { status: statusPick });
  ...
  const handleDelete = async () => {
    ...
    setBusy(true);
    try {
      const res = await accountApi.bulkDelete(ids);
      toast.success(res.message); onClear(); onRefresh();
    } catch (e) { toast.error(e.message); }
    finally { setBusy(false); }
  };
  ...
  const handlePromote = async () => {
    setPromoting(true);
    try {
      const res = await accountApi.promoteEligible();
  ...
  const BB = ({ icon, label, onClick, color }) => (
    <button onClick={onClick} disabled={anyBusy} style={{
      ...
      opacity: anyBusy ? 0.65 : 1, transition: 'opacity .15s',
    }}>{icon} {label}</button>
  );
  ...
  return (
    <div style={{
      ...
      <BB icon="🎯" label={promoting ? 'Đang chuyển...' : 'Chuyển đủ ĐK'} onClick={handlePromote} color="#8b5cf6" />
...
// ── Copy Unused Bar (DU_DK tab) ───────────────────────────────────────────────
function CopyUnusedBar({ onRefresh }) {
  const [busy,      setBusy]      = useState(false);
  const [lastCount, setLastCount] = useState(null);

  const handle = async (markUsed) => {
    setBusy(true);
    try {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#fff', fontWeight: 600, fontSize: '.9rem' }}>
        🎯 Accounts Đủ Điều Kiện {lastCount != null && `· Lần trước: ${lastCount} acc`}
      </div>
      <div style={{ flex: 1 }} />
      <button onClick={() => handle(false)} disabled={busy} style={{
        background: 'rgba(255,255,255,.2)', border: '1px solid rgba(255,255,255,.4)',
        color: '#fff', borderRadius: '8px', padding: '.45rem 1rem',
        cursor: busy ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '.85rem',
      }}>📋 Copy chưa dùng</button>
      <button onClick={() => handle(true)} disabled={busy} style={{
        background: '#fff', border: 'none', color: '#7c3aed',
        borderRadius: '8px', padding: '.45rem 1rem',
        cursor: busy ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '.85rem',
      }}>📋✅ Copy & Đánh dấu</button>
    </div>
  );
}
...
// ── Quick bar cho DANG_UP ────────────────────────────────────────────────────
function DangUpQuickBar({ onFilter, onClearFilter, videoMax }) {
  const color = '#10b981';
  return (
    <div style={{
      ...
      {videoMax !== '' && (
        <button onClick={onClearFilter} style={{ background: 'transparent', border: '1px solid #334155', color: '#94a3b8', borderRadius: '6px', padding: '.35rem .6rem', cursor: 'pointer', fontSize: '.78rem' }}>✕ Bỏ lọc</button>
      )}
      <span style={{ color: '#475569', fontSize: '.75rem' }}>→ Chọn tất cả rồi dùng BulkBar để copy</span>
    </div>
  );
}

// ── Kháng Quick Bar (ACC_DA_KHANG / ACC_CHUA_KHANG) ─────────────────────────
function KhangQuickBar({ status, onFilter, onClearFilter, videoMax }) {
  ...
  return (
    <div style={{
      ...
      padding: '.7rem 1.25rem', marginBottom: '1rem',
      display: 'flex', alignItems: 'center', gap: '.75rem', flexWrap: 'wrap',
    }}>
      ...
      {videoMax !== '' && (
        <button onClick={onClearFilter} style={{ background: 'transparent', border: '1px solid #334155', color: '#94a3b8', borderRadius: '6px', padding: '.35rem .6rem', cursor: 'pointer', fontSize: '.78rem' }}>✕ Bỏ lọc video</button>
      )}
      <span style={{ color: '#475569', fontSize: '.75rem' }}>→ Chọn tất cả rồi dùng BulkBar để copy hoặc đổi trạng thái</span>
    </div>
  );
}
...
function ImportModal({ onClose, onImported, groups = [], onGroupCreated }) {
  ...
  const [status,    setStatus]    = useState('LOGIN_THANH_CONG');
  ...
  const handle = async () => {
    ...
    try {
      const res = await accountApi.import(text, status, groupId || null);
      setResult(res.data);
      toast.success(res.message);
      onImported();
    } catch (e) { toast.error(e.message); }
      ...
      <div style={{ background: '#1e293b', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '560px', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value)} style={{ padding: '.5rem .75rem', borderRadius: '8px', border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', width: '100%' }}>
            <option value="LOGIN_THANH_CONG">📤 Upload Thành Công</option>
            <option value="ACC_DA_KHANG">🛡️ Đã Kháng</option>
            <option value="ACC_CHUA_KHANG">⚠️ Chưa Kháng</option>
          </select>
        </div>
        <div style={{ marginBottom: '1rem' }}>
          <AccountGroupPicker accountType="app" groups={groups} value={groupId} onChange={setGroupId} onGroupsChanged={onGroupCreated} />
        </div>
        <textarea
        ...
        <div style={{ display: 'flex', gap: '.75rem', marginTop: '1rem' }}>
          <button onClick={handle} disabled={importing} style={{ flex: 1, background: importing ? '#334155' : '#3b82f6', border: 'none', color: '#fff', borderRadius: '8px', padding: '.65rem', cursor: importing ? 'not-allowed' : 'pointer', fontWeight: 700 }}>
...
export default function AccountList() {
  ...
  const handleCheckLive = async (ids) => {
    ...
    try {
      const { rows, live, die, unknown } = await checkLiveInBatches('/accounts/check-live', ids, settings, setClProgress);
  ...
  return (
    <div className="page">
      <style>{`
        ...
        .upload-row:hover { background: rgba(59,130,246,.05) !important; cursor: default; }
        .upload-row.row-selected { background: rgba(59,130,246,.08) !important; }
          ...
          <div className="filter-row">
            ...
            <div className="filter-group">
              ...
              <input type="number" min={0} placeholder="0" value={filters.video_min}
                onChange={(e) => setFilter('video_min', e.target.value)} style={{ minWidth: '70px', maxWidth: '80px' }} />
            </div>
            <div className="filter-group">
      ...
      {currentStatus === 'ACC_DU_DK' && (
        <CopyUnusedBar onRefresh={() => fetchAccounts(filters)} />
      )}
        ...
        <div className="table-container">
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <button onClick={() => setShowImport(true)} style={{ marginTop: '1rem', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '8px', padding: '.6rem 1.5rem', cursor: 'pointer', fontWeight: 600 }}>
                  ...
                  return (
                    <tr key={acc.id} className={`upload-row${selected.has(acc.id) ? ' row-selected' : ''}`}>
                      ...
                      <td>
                        ...
                        {acc.fail_reason && <div style={{ fontSize: '.7rem', color: '#ef4444', marginTop: 2 }}>⚠️ {acc.fail_reason}</div>}
                      </td>
                      ...
                      <td style={{ color: acc.video_count > 0 ? '#047857' : '#64748b', fontWeight: acc.video_count >= eligibility.min_videos ? 800 : 700 }}>
                        {acc.video_count ?? 0}
                        {acc.video_count >= eligibility.min_videos && <span style={{ color: '#22c55e', marginLeft: '.3rem', fontSize: '.7rem' }}>✓</span>}
                      </td>
